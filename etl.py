...
def carregar_dados(df: pd.DataFrame, format_saida: list): # Aqui criamos um parâmetro onde o output vai poder ser um csv, parquet ou os dois. Esse def vai ter uma caracteristica de procedure, porque ele não vai ter return, queremos só que ela salve.
    for formato in format_saida: # Aqui tivemos que criar um for para interar sobre os metodos de saida.
        if format == 'csv':
            df.to_csv('dados.csv')
        if format == 'parquet':
